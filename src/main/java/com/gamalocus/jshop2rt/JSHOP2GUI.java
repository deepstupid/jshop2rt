...
public class JSHOP2GUI extends JFrame {
    ...
    private static ArrayList<PlanStepInfo> planStepList = new ArrayList();
...
//-----------------------------------------------------------------------------
    ...
    private TextArea stepInfoTextArea;

    // The text input box used to specifiy the step interval length
    private JTextField multiStepField;
    ...
    private Hashtable<Integer, DefaultMutableTreeNode> treeNodeReferences;
    ...
    private LeafTrackerDialog leafTracker;
...
//-----------------------------------------------------------------------------
    ...
    private void initFieldsAndCreateInterface() {
        ...
        singleStepButton.addActionListener(e -> runOneStep());


        JButton multiStepButton = new JButton("Multi-Step");
        multiStepButton.addActionListener(e -> {
            int numSteps = Integer.valueOf(multiStepField.getText());
    ...
    private boolean runOneStep() {
        ...
        if (iterator < planStepList.size()) {
            ...
            // trying a task
                ...
                if (iterator - 2 >= 0)
                    if (planStepList.get(iterator - 2).planFound)
                        leafNodes.clear();
...
//-----------------------------------------------------------------------------  
    ...
    private void processPlanFound() {
        planNumber++;
        ArrayList<String> plan = new ArrayList<>(); // vector of strings containing the leaves' numbered names
...
//-----------------------------------------------------------------------------
    ...
    private DefaultMutableTreeNode processReduced(PlanStepInfo step, ArrayList<Node> toAdd) {
        ...
        // set messageLabel
        StringBuilder msg = new StringBuilder("Reduced ");
        msg.append(step.taskAtom);
        msg.append(" into the following: ");
...
//-----------------------------------------------------------------------------
     ...
     * Listener Classes
     */
    private static class SHOP2GUIWindowAdapter extends WindowAdapter {
...
//-----------------------------------------------------------------------------
    ...
    private static class Node {
        ...
        String getState() {
            ...
            if (state == null)
                retval = new StringBuilder("Unknown");
                // the state is empty
            else if (state.size() == 0) {
                retval = new StringBuilder();
            } else {
                retval = new StringBuilder(state.get(0));
                for (int i = 1; i < state.size(); i++) {
                    retval.append("\n");
                    retval.append(state.get(i));
                }
            }
...
//-----------------------------------------------------------------------------
    ...
    private static class NodeRenderer extends DefaultTreeCellRenderer {
        ...
        final ImageIcon yellowBall;
        final ImageIcon blueBall;
        final ImageIcon smYellowBall;
        final ImageIcon smBlueBall;
...
//-----------------------------------------------------------------------------
    ...
    private class PlanDialog extends JDialog {
        ...
        PlanDialog(String title, ArrayList<String> plan) {
            setTitle(title);
...
//-----------------------------------------------------------------------------
    ...
    private class StateWindowDialog extends JDialog {
        ...
        StateWindowDialog(String title) {
            setTitle(title);
            getContentPane().setLayout(new FlowLayout(FlowLayout.CENTER, 5, 20));
...
//-----------------------------------------------------------------------------
    ...
    private class LeafTrackerDialog extends JDialog {
        ...
        private final Label leafTotalLabel;
        private final TextField leafNumberField;

        LeafTrackerDialog(Frame parent) {
            ...
            prevButton.addActionListener(e -> {
                int leafNum = Integer.valueOf(leafNumberField.getText());
                leafNum--;
