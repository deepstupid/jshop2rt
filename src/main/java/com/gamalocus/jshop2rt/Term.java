 ...
 * @version 1.0.3
 */
public abstract class Term extends CompileTimeObject implements Serializable {
    private final static Logger logger = Logger.getLogger(Term.class.getName());
     ...
     *
     * @param inp     the bindings to be merged. This is a two dimensional array, the
     *                first dimension indexes the different bindings, and the second
     *                dimension indexes variables within each binding.
     * @param howMany how many of available bindings we want to merge. Bindings indexed
            ...
            for (int j = 0; j < howMany; j++) {
                ...
                if (inp[j] != null) {
                    ...
                    if (logger.isLoggable(Level.WARNING)) {
                        ...
                        buf.append("Found null binding. Remaining bindings:");
                        for (int k = 0; k < inp.length; k++) {
                            buf.append("\n\tinp[").append(k).append("]=").append(Arrays.toString(inp[k]));
                        }
                        logger.log(Level.WARNING, buf.toString(), new Throwable());
                    }
                }
            }
        }
    ...
    /**
     * Terms must implement hashCode to be able to use hash sets for rapid add and removal to
     * world state.
     */
