 ...
 * @version 1.0.3
 */
public class TaskList extends CompileTimeObject implements Serializable {
    /**
     ...
     */
    public static final TaskList empty = new TaskList();
    private static final long serialVersionUID = 6222490586644466537L;
    /**
     ...
     *
     * @param size      the number of task lists in the task list this object represents.
     ...
     *
     * @param subtasksIn a <code>Vector</code> of subtasks the output task list will
     *                   represent.
     ...
     *
     * @return a <code>LinkedList</code> of the task atoms we have the option to
     * achieve right now.
     */
    public LinkedList<TaskList> getFirst() {
        //-- Make an empty list.
        LinkedList<TaskList> retVal = new LinkedList<>();
     ...
     *
     * @param what   the <code>String</code> name of the task list created by this
     *               piece of code.
     * @param string
    ...
    public String getInitCode(String label, String what) {
        ...
        s = new StringBuilder("\t\t\t" + what + " = new TaskList(" + subtasks.length + ", " + ordered
                + ");" + endl);

        //-- Recursively create subtasks.
        for (int i = 0; i < subtasks.length; i++)
            s.append(subtasks[i].getInitCode(String.format("Sub-list of %s", label), what + ".subtasks[" + i + "]"));
    ...
    public void setVarCount(int varCountIn) {
        ...
        if (subtasks == null)
            ...
            for (TaskList subtask : subtasks) subtask.setVarCount(varCountIn);
    }
    ...
    public TaskList clone() {
        TaskList retval = new TaskList(task);
     ...
     * &lt;Added 5/26/06&gt;
     */
    private void getChildren(ArrayList<String> childrenList) {
        ...
        if (!isEmpty()) {
            ...
            if (subtasks == null)
                ...
                for (TaskList subtask : subtasks) {
                    if (!subtask.isEmpty()) {
                        //-- Add the subtask to the result.
                        subtask.getChildren(childrenList);
                    }
                }
            }
        }
