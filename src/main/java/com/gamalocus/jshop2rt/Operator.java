...
public abstract class Operator extends DomainElement {
     ...
     */
    protected final int addVarIdx;
    /**
     * Cost of this operator.
     */
    protected final Term cost;
    /**
     * Represents the delete list in case it is a variable (The integer value
     * represents variable's index), otherwise it is -1.
     */
    protected final int delVarIdx;
    /**
     * Represents the add list in case it is a real list and not a variable.
     */
    protected DelAddElement[] add;
    /**
     * Represents the delete list in case it is a real list and not a variable.
     */
    protected DelAddElement[] del;

    /**
     * To initialize the operator.
     *
     * @param owner       Domain instance owning this element.
     * @param head        head of the operator.
     * @param delVarIdxIn the index of the delete list as a variable, -1 if the delete list
     *                    is not a variable.
     * @param addVarIdxIn the index of the add list as a variable, -1 if the add list is
     *                    not a variable.
     * @param costIn      the cost of the operator.
     */
    public Operator(Domain owner, Predicate head, int delVarIdxIn, int addVarIdxIn,
                    Term costIn) {
        super(owner, head);
     ...
     * @param delAdd  a 4-member array of type <code>Vector</code> used to keep track
    ...
    public boolean apply(Term[] binding, State s, Vector[] delAdd) {
        ...
        //-- If the delete list is a real list,
            ...
            //-- Try to delete the atom from the current state of the world.
            for (DelAddElement delAddElement : del)
                if (!delAddElement.del(s, binding, delAdd))
        ...
        //-- If the add list is a real list,
            ...
            for (DelAddElement delAddElement : add) delAddElement.add(s, binding, delAdd);
        }
     ...
     * @return term describing the cost of applying this operator.
     */
    public Term getCost(Term[] binding) {
        return cost.bind(binding);
    }
