...
public class PlanStepInfo {
    ...
    public String toString(Domain domain) {
        ...
        if (taskAtom != null) {
            buf.append("Task Atom: ").append(taskAtom.toString(domain)).append("\n");
        }
        ...
        if (preconditions.length() != 0) {
            buf.append("Preconditions: ").append(preconditions).append("\n");
        }
        if (children != null) {
            buf.append("Children: \n");
            for (TaskList child : children) buf.append(child.toString(domain)).append("\n");
        }

        if (state != null) {
            buf.append("State: ");
            for (String s : state) buf.append("\n\t").append(s).append(" ");
            buf.append("\n");
        }
